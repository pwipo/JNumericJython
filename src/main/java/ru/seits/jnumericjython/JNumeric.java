...
public class JNumeric extends PyObject implements ClassDictInit {
    ...
    public static void classDictInit(final PyObject dict) {
        ...
        dict.__setitem__("pi", Py.newFloat(Math.PI));
        dict.__setitem__("e", Py.newFloat(Math.E));
        ...
        dict.__setitem__("mean", JNumeric.mean);
    }
     ...
     * mean(a, [axis])
     */
    static final public PyObject mean = new MeanFunction();
...
final class TransposeFunction extends KeywordFunction {
    ...
    @Override
    public PyObject _call(final PyObject args[]) {
        return PyMultiarray.transpose(args[0], PyMultiarray._getAxes(args[0], args[1]));
    }
}
...
final class MeanFunction extends KeywordFunction {
    private static final long serialVersionUID = -5098199415958538661L;

    MeanFunction() {
        this.docString = "mean(a, [axis])";
        this.argNames = new String[]{"a", "axis"};
        this.defaultArgs = new PyObject[]{null, null};
    }
    ...
    public PyObject _call(final PyObject args[]) {
        ...
        // final int axis = Py.py2int(args[1]);
        int length = Array.getLength(a.data);
        // if (args[1] == null) {
        double sum = 0;
        for (int i = 0; i < length; i++)
            sum += Array.getDouble(a.data, i);
        return Py.java2py(sum / length);
        // } else {
        //     // todo: need implement
        //     return a;
        // }
