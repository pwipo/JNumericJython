...
public class PyMultiarray extends PySequence {
    ...
    public static PyMultiarray arrayRange(PyObject start, PyObject stop, final PyObject step, char typecode) {
        ...
        if (typecode != 'O') {
            return PyMultiarray.arrayRange(start.__float__()
    ...
    public PyMultiarray reshape(PyObject shape) {
        return reshape(this, PyMultiarray.objectToInts(shape, false));
    }
    ...
    public void resize(PyObject shape) {
        PyMultiarray a = resize(this, PyMultiarray.objectToInts(shape, false));
        _setOwn(a);
    }
    ...
    private void _setOwn(PyMultiarray a) {
        this.data = a.data;
    ...
    public void sort(final int axis) {
        PyMultiarray a = sort(this, axis);
        _setOwn(a);
    }
    ...
    public static int[] _getAxes(PyObject a, PyObject axesObj) {
        int[] axes;
        // Move some of this to PyMultiarray?
        if (axesObj == Py.None) {
            axes = new int[PyMultiarray.shapeOf(a).length];
            for (int i = 0; i < axes.length; i++) {
                axes[i] = axes.length - 1 - i;
            }
        } else {
            axes = new int[axesObj.__len__()];
            for (int i = 0; i < axes.length; i++) {
                axes[i] = Py.py2int(axesObj.__getitem__(i));
            }
        }
        return axes;
    }

    public PyMultiarray transpose(PyObject axesObj) {
        return transpose(this, _getAxes(this, axesObj));
    }
    ...
    public PyMultiarray repeat(final PyObject oRepeats, int axis) {
        return repeat(this, oRepeats, axis);
    }
    ...
    public PyMultiarray take(final PyObject oIndices, int axis) {
        return take(this, oIndices, axis);
    }
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < bs[i].dimensions.length; j++) {
                if (a.dimensions[j + a.dimensions.length
                        - bs[i].dimensions.length] != bs[i].dimensions[j]) {
                    throw Py
    ...
    public static PyMultiarray concatenate(final PyObject po, int axis) {
        if (po.__len__() == 0) {
            return PyMultiarray.zeros(new int[]{}, 'i');
        }
        // Check axis and rotate the axis to zero.
    ...
    public PyObject searchSorted(final PyObject v) {
    ...
    public final static int[] shapeOf(final PyObject seq) {
        if (seq instanceof PyMultiarray) {
            return ((PyMultiarray) seq).dimensions
                    .clone();
        }
    ...
    final static char commonType(final char a, final char b) {
        ...
        if (newtype == PyMultiarray.PYOBJECT) {
            return 'O';
        }
     ...
     */
    @Override
    public Object __tojava__(final Class<?> c) {
     ...
     */
    @Override
    public PyObject __getitem__(final int index) {
     ...
     */
    @Override
    public int __cmp__(final PyObject other) {
        final boolean debug_this = false;
     ...
     */
    @Override
    public PyObject __getitem__(final PyObject indices) {
     ...
     */
    @Override
    public void __setitem__(final PyObject indices, final PyObject pyValue) {
     ...
     */
    @Override
    protected void setslice(final int start, final int stop, final int step, final PyObject value) {
     ...
     */
    @Override
    protected PyObject getslice(final int start, final int stop, final int step) {
        final PyObject startObject = (start >= 0) ? Py.newInteger(start)
                : Py.None;
        final PyObject stopObject = (stop >= 0) ? Py.newInteger(stop) : Py.None;
    ...
    public PyObject __findattr_ex__(final String name) {
        ...
        if (name == "__doc__") {
            return Py.newString(this.docString);
        }
        if (name == "shape") {
            return PyTuple.fromIterable(new PyArray(
                    int.class,
    ...
    @Override
    protected PyObject pyget(final int i) {
        return this.get(i);
    }

    @Override
    protected PyObject repeat(final int count) {
    ...
    @Override
    public PyObject __radd__(final PyObject po) {
        return this.__add__(po);
    }

    @Override
    public PyObject __rsub__(final PyObject po) {
        return PyMultiarray.array(po).__sub__(this);
    }

    @Override
    public PyObject __rmul__(final PyObject po) {
        return this.__mul__(po);
    }

    @Override
    public PyObject __rdiv__(final PyObject po) {
        return PyMultiarray.array(po).__div__(this);
    }

    @Override
    public PyObject __rmod__(final PyObject po) {
        return PyMultiarray.array(po).__mod__(this);
    }

    @Override
    public PyObject __divmod__(final PyObject po) {
    ...
    @Override
    public PyObject __rdivmod__(final PyObject po) {
    ...
    @Override
    public PyObject __add__(final PyObject o) {
    ...
    @Override
    public PyObject __sub__(final PyObject o) {
    ...
    @Override
    public PyObject __mul__(final PyObject o) {
    ...
    @Override
    public PyObject __div__(final PyObject o) {
    ...
    @Override
    public PyObject __mod__(final PyObject o) {
    ...
    @Override
    public PyObject __pow__(final PyObject o) {
    ...
    @Override
    public PyObject __and__(final PyObject o) {
    ...
    @Override
    public PyObject __or__(final PyObject o) {
    ...
    @Override
    public PyObject __xor__(final PyObject o) {
